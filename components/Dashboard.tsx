...
import { 
  TrendingUp, Users as UsersIcon, Sparkles, Loader2, Clock, Zap, Plus, X, ConciergeBell, Eye, ArrowUpRight, Building2, CheckCircle, CreditCard, DollarSign, UserCheck, ArrowRight, History, Percent, LayoutGrid, Activity, Wallet, BellRing, CheckCircle2, MapPin, User, Globe, CalendarRange, DoorOpen
} from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ state, onAddService, onUpdateBooking, onOpenDetails, onTabChange, onQuickSettle, onFulfillService }) => {
  ...
  const nextCheckIn = useMemo(() => {
    ...
    return state.bookings
      .filter(b => (b.status === 'confirmed' || b.status === 'pending') && b.startDate >= today)
      .sort((a, b) => a.startDate.localeCompare(b.startDate))[0];
  }, [state.bookings]);

  const platformPerformance = useMemo(() => {
    const today = new Date().toISOString().split('T')[0];
    const stats: Record<string, { past: number, now: number, next: number }> = {};
    PLATFORMS.forEach(p => stats[p] = { past: 0, now: 0, next: 0 });
    state.bookings.filter(b => b.status !== 'cancelled' && b.status !== 'maintenance').forEach(b => {
      const p = b.platform || 'Direct';
      if (!stats[p]) stats[p] = { past: 0, now: 0, next: 0 };
      if (b.endDate < today) stats[p].past++;
      else if (b.startDate <= today && b.endDate >= today) stats[p].now++;
      else if (b.startDate > today) stats[p].next++;
    });
    return stats;
  }, [state.bookings]);

  const stats = [
    { label: 'Total Inventory', value: state.apartments.length, icon: Building2, color: 'text-slate-950', bg: 'bg-slate-100', tab: 'apartments' },
    { label: 'Active Folios', value: state.bookings.filter(b => b.status !== 'cancelled').length, icon: ConciergeBell, color: 'text-sky-600', bg: 'bg-sky-50', tab: 'bookings' },
    { label: 'Current Guests', value: state.bookings.filter(b => b.status === 'stay').length, icon: UserCheck, color: 'text-emerald-600', bg: 'bg-emerald-50', tab: 'calendar' },
    { label: 'Units Ready', value: state.apartments.length - state.bookings.filter(b => b.status === 'stay' || b.status === 'maintenance').length, icon: DoorOpen, color: 'text-amber-600', bg: 'bg-amber-50', tab: 'apartments' },
  ];
  ...
  const handleMarkAllFulfilled = () => {
    ...
    b.services.forEach(sid => { if (!(b.fulfilledServices || []).includes(sid)) onFulfillService(b.id, sid, false); });
    (b.extraServices || []).forEach(es => { if (!es.isFulfilled) onFulfillService(b.id, es.id, true); });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
        ...
        <div>
           <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-slate-400 font-black text-[9px] uppercase tracking-[0.3em] flex items-center gap-2">
                Bahia Intelligence Terminal v15.8 
                <span className="flex items-center gap-1 text-sky-600 border border-sky-100 bg-sky-50 px-1.5 py-0.5 rounded text-[7px]"><Activity className="w-2 h-2" /> Live Operational Matrix</span>
              </p>
           </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          <button 
            ...
            className="bg-white p-5 rounded-[2rem] border border-slate-200 shadow-sm group hover:border-sky-500 hover:scale-[1.02] transition-all text-left"
          >
            <div className={`w-10 h-10 ${stat.bg} ${stat.color} rounded-xl flex items-center justify-center mb-4 group-hover:bg-slate-950 group-hover:text-white transition-colors`}>
      ...
      {/* Spotlight: Next Check-In */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
           <div className="bg-slate-950 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group border-b-8 border-sky-500">
              <div className="absolute top-0 right-0 p-10 opacity-5 group-hover:opacity-10 transition-opacity"><DoorOpen className="w-48 h-48" /></div>
              <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                 <div>
                    <p className="text-sky-400 font-black text-[10px] uppercase tracking-[0.4em] mb-4">Next Scheduled Arrival</p>
                    {nextCheckIn ? (
                      <>
                        ...
                        <div className="flex flex-wrap gap-4 mt-6">
                           <div className="px-4 py-2 bg-white/10 rounded-xl border border-white/10 flex items-center gap-2">
                              ...
                              <span className="text-xs font-black">Unit {state.apartments.find(a => a.id === nextCheckIn.apartmentId)?.unitNumber}</span>
                           </div>
                           <div className="px-4 py-2 bg-white/10 rounded-xl border border-white/10 flex items-center gap-2">
                              <Clock className="w-4 h-4 text-emerald-400" />
                              <span className="text-xs font-black">{nextCheckIn.startDate} @ {nextCheckIn.checkInTime || '14:00'}</span>
                           </div>
                           <div className="px-4 py-2 bg-sky-500 text-white rounded-xl shadow-lg flex items-center gap-2">
                              <Globe className="w-4 h-4" />
                              <span className="text-xs font-black uppercase tracking-widest">{nextCheckIn.platform}</span>
                           </div>
                        </div>
                      </>
                    ) : (
                      <p className="text-2xl font-black text-slate-600">No upcoming arrivals</p>
                    )}
                 </div>
                 {nextCheckIn && (
                    <button 
                      onClick={() => onOpenDetails(nextCheckIn.id)}
                      className="bg-white text-slate-950 p-6 rounded-[2rem] shadow-xl hover:bg-sky-400 hover:text-white transition-all group/btn"
                    >
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col justify-between">
           <div className="space-y-4">
              <h3 className="text-sm font-black uppercase tracking-tighter text-slate-950 flex items-center gap-2">
                <LayoutGrid className="w-4 h-4 text-sky-500" /> Quick Operations
              </h3>
              <p className="text-[10px] text-slate-400 font-bold leading-relaxed">Direct links to critical management sectors</p>
           </div>
           <div className="grid grid-cols-2 gap-3 mt-6">
              <button onClick={() => onTabChange?.('apartments')} className="p-4 bg-slate-50 hover:bg-slate-950 hover:text-white rounded-2xl transition-all flex flex-col items-center gap-2 group">
              ...
              <button onClick={() => onTabChange?.('bookings')} className="p-4 bg-slate-50 hover:bg-slate-950 hover:text-white rounded-2xl transition-all flex flex-col items-center gap-2 group">
                 <ConciergeBell className="w-5 h-5 text-slate-400 group-hover:text-sky-400" />
                 <span className="text-[8px] font-black uppercase">Folios</span>
              </button>
              <button onClick={() => onTabChange?.('calendar')} className="p-4 bg-slate-50 hover:bg-slate-950 hover:text-white rounded-2xl transition-all flex flex-col items-center gap-2 group">
                 <CalendarRange className="w-5 h-5 text-slate-400 group-hover:text-sky-400" />
                 <span className="text-[8px] font-black uppercase">Timeline</span>
              </button>
              <button onClick={() => onTabChange?.('services')} className="p-4 bg-slate-50 hover:bg-slate-950 hover:text-white rounded-2xl transition-all flex flex-col items-center gap-2 group">
                 <Zap className="w-5 h-5 text-slate-400 group-hover:text-sky-400" />
          ...
          <div className="bg-amber-50 border border-amber-100 p-6 rounded-[2.5rem] shadow-sm">
             <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                   ...
                   <h3 className="text-xs font-black uppercase tracking-tighter text-amber-900">Amenity Delivery Queue</h3>
                </div>
             </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-sm font-black uppercase text-slate-900 tracking-tighter">Live Updates</h3>
            </div>
