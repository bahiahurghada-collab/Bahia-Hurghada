...
const MaintenanceManagement: React.FC<MaintenanceManagementProps> = ({ expenses, apartments, onAddExpense, onDeleteExpense }) => {
  ...
  const [formData, setFormData] = useState<Omit<Expense, 'id'>>({
    ...
    amount: 0,
    currency: 'EGP',
    apartmentId: '' // Default to general
  });
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-white p-10 rounded-[3rem] border-2 border-slate-100 shadow-xl">
        <div>
          <h2 className="text-4xl font-black text-slate-950 tracking-tighter uppercase">Operations Ledger</h2>
      ...
      <div className="grid grid-cols-1 gap-6">
        {expenses.length > 0 ? expenses.map(e => {
          ...
          return (
            <div key={e.id} className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 flex items-center justify-between group hover:border-rose-200 transition-all">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <h4 className="text-xl font-black text-slate-950">{e.description}</h4>
                </div>
              </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-[200] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] p-10 max-w-2xl w-full shadow-2xl animate-in zoom-in-95 duration-300 border-4 border-slate-900">
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-rose-600 text-white rounded-2xl shadow-lg"><ShoppingCart className="w-8 h-8" /></div>
                <h3 className="text-3xl font-black text-slate-950 tracking-tighter uppercase">Log Expense</h3>
              </div>
              <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-slate-100 rounded-full"><X className="w-10 h-10 text-slate-950" /></button>
            </div>
              ...
              <div className="space-y-6 bg-slate-50 p-8 rounded-[2rem] border-2 border-slate-100">
                <div className="grid grid-cols-2 gap-6">
                   <div className="space-y-2">
                     ...
                     <select className="w-full p-4 rounded-xl border-2 border-white bg-white font-bold" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value as any})}>
                   ...
                   <div className="space-y-2">
                     ...
                     <select className="w-full p-4 rounded-xl border-2 border-white bg-white font-bold" value={formData.apartmentId} onChange={e => setFormData({...formData, apartmentId: e.target.value})}>
                ...
                <div className="space-y-2">
                  ...
                  <input required className="w-full p-4 rounded-xl border-2 border-white bg-white font-bold" placeholder="What was purchased?" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
                </div>
                
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2 col-span-1">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Date</label>
                    <input type="date" required className="w-full p-4 rounded-xl border-2 border-white bg-white font-bold" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
                  </div>
                  <div className="space-y-2 col-span-1">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Amount</label>
                    <input type="number" required className="w-full p-4 rounded-xl border-2 border-white bg-white font-black text-xl text-rose-600 outline-none" value={formData.amount || ''} onChange={e => setFormData({...formData, amount: Number(e.target.value)})} />
                  </div>
                  <div className="space-y-2 col-span-1">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Currency</label>
                    <select className="w-full p-4 rounded-xl border-2 border-white bg-white font-bold" value={formData.currency} onChange={e => setFormData({...formData, currency: e.target.value as Currency})}>
