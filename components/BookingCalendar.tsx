...
const BookingCalendar: React.FC<CalendarProps> = ({ apartments, bookings, onBookingInitiate, onEditBooking, state }) => {
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500 font-bold">
      <div className="bg-white p-6 rounded-[2.5rem] border-2 border-slate-950 shadow-2xl flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-4">
          <div className="p-4 bg-slate-950 rounded-2xl text-white shadow-xl shadow-slate-950/20"><CalIcon className="w-6 h-6" /></div>
          <div>
        ...
        <div className="flex items-center gap-4">
          {selection.start && (
            <div className="flex items-center gap-3 animate-in slide-in-from-right-4">
              <div className="px-6 py-3 bg-slate-950 text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 bg-slate-100 p-1.5 rounded-2xl">
            ...
            <button onClick={() => setCurrentMonth(new Date(today.getFullYear(), today.getMonth(), 1))} className="px-6 py-2 bg-slate-950 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl transition-all hover:bg-sky-600">Today</button>
      ...
      <div className="bg-white rounded-[4rem] border-2 border-slate-950 shadow-2xl overflow-hidden relative">
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-950 text-white">
                <th className="sticky left-0 z-40 bg-slate-950 p-6 min-w-[160px] border-r border-white/10 text-left text-xs uppercase tracking-widest opacity-60">Apartment Asset</th>
                {days.map(day => {
                  const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
                  const isToday = dateStr === todayStr;
                  return (
                    <th key={day} className={`p-4 min-w-[65px] border-r border-white/5 text-center transition-all ${isToday ? 'bg-sky-500 shadow-[inset_0_0_20px_rgba(0,0,0,0.1)]' : 'text-slate-500'}`}>
            ...
            <tbody>
              {apartments.map(apt => (
                <tr key={apt.id} className="group border-b border-slate-100 last:border-0">
                  <td className="sticky left-0 z-30 bg-white p-6 font-black text-slate-950 border-r border-slate-200 shadow-lg flex items-center gap-3">
                    <div className="p-2 bg-slate-50 rounded-lg group-hover:bg-slate-950 group-hover:text-white transition-all duration-500">
                      ...
                      <td key={day} onClick={() => handleSlotClick(apt.id, day)} className={`h-20 border-r border-slate-100 cursor-pointer relative group/cell transition-all ${isToday ? 'bg-sky-50/50' : 'hover:bg-slate-50'}`}>
                        {booking && (
                          <div className={`absolute inset-y-1 left-0 right-0 z-10 mx-1 rounded-xl flex items-center px-3 overflow-hidden shadow-2xl border-b-4 transition-all duration-300 group-hover/cell:scale-[1.03] group-hover/cell:z-20 ${
                            booking.status === 'stay' ? 'bg-emerald-600 text-white border-emerald-800' : 'bg-slate-950 text-white border-slate-700'
                          }`}>
                            <span className="text-[9px] font-black uppercase truncate leading-none tracking-tighter">
                               {isStart ? '● START' : '•'}
                            </span>

                            {/* ELITE HIGH-CONTRAST STABLE TOOLTIP */}
                            <div className="absolute top-[-190px] left-1/2 -translate-x-1/2 w-72 bg-slate-950 border-4 border-white p-6 rounded-[2.5rem] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.8)] opacity-0 invisible group-hover/cell:opacity-100 group-hover/cell:visible transition-all z-[100] text-white pointer-events-none scale-90 group-hover/cell:scale-100">
                               ...
                               <div className="space-y-3">
                                  <div className="flex justify-between items-center text-[11px] font-black">
                                     <span className="text-slate-500 uppercase tracking-widest">Total Valuation</span>
                                  ...
                                  <div className="flex justify-between items-center text-[10px] font-black">
                                     <span className="text-slate-500 uppercase tracking-widest">Balance Due</span>
                                     <span className="text-rose-500">{(booking.totalAmount - booking.paidAmount).toLocaleString()} {booking.currency}</span>
                                  </div>
                               ...
                               <div className="absolute bottom-[-10px] left-1/2 -translate-x-1/2 w-5 h-5 bg-slate-950 border-r-4 border-b-4 border-white rotate-45"></div>
                            </div>
                          </div>
                        )}
                      </td>
