  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-white p-10 rounded-[3rem] border-2 border-slate-100 shadow-xl">
        <div>
          <h2 className="text-4xl font-black text-slate-950 tracking-tighter uppercase">Team Authority</h2>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              <th className="px-10 py-8 text-xs font-black text-slate-500 uppercase tracking-widest">Operator Name</th>
              <th className="px-10 py-8 text-xs font-black text-slate-500 uppercase tracking-widest">Access Key</th>
              <th className="px-10 py-8 text-xs font-black text-slate-500 uppercase tracking-widest">Global Role</th>
              <th className="px-10 py-8 text-right text-xs font-black text-slate-500 uppercase tracking-widest">Security Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
            {users.map(u => (
              <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-10 py-6 font-black text-xl text-slate-900 uppercase tracking-tight">{u.name}</td>
                ...
                <td className="px-10 py-6 text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    {u.username !== 'admin' && (
                      <button onClick={() => { if(window.confirm('Erase staff record?')) onDeleteUser(u.id); }} className="p-3 text-rose-600 hover:bg-rose-50 rounded-xl transition-all border-2 border-rose-50"><Trash2 className="w-5 h-5" /></button>
                    )}
                  </div>
                </td>
              </tr>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-md z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] p-10 max-w-5xl w-full shadow-2xl animate-in zoom-in-95 duration-300 border-4 border-slate-900 max-h-[92vh] overflow-y-auto custom-scrollbar">
            <div className="flex justify-between items-center mb-8 sticky top-0 bg-white z-10 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-4">
                <div className="p-3 bg-slate-950 text-white rounded-2xl shadow-lg"><LockKeyhole className="w-8 h-8" /></div>
                <h3 className="text-3xl font-black text-slate-950 tracking-tighter uppercase">Access Permissions Matrix</h3>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-8 rounded-[2rem] border-2 border-slate-100">
                <div className="space-y-2">
                  ...
                  <input required className="w-full p-4 rounded-xl border-2 border-white bg-white font-bold" placeholder="Full Employee Name" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">System Login ID</label>
                  <input required className="w-full p-4 rounded-xl border-2 border-white bg-white font-bold" placeholder="Unique Username" value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} />
                </div>
                <div className="space-y-2">
              ...
              <div className="space-y-8 bg-white border-2 border-slate-100 rounded-[2rem] p-8">
                <div className="flex justify-between items-center border-b pb-6">
                   <div>
                      <h4 className="text-2xl font-black text-slate-900 tracking-tighter uppercase">Permissions Granularity</h4>
                  ...
                  {permissionGroups.map((group, idx) => (
                    <div key={idx} className="space-y-4">
                      <div className="flex items-center gap-3 border-b border-slate-50 pb-2">
                        <div className="p-2 bg-slate-950 text-white rounded-lg"><group.icon className="w-4 h-4" /></div>
                        <span className="text-xs font-black uppercase tracking-[0.2em] text-slate-900">{group.title}</span>
                      </div>
                      <div className="space-y-2">
              ...
              <button type="submit" className="w-full py-8 bg-slate-950 text-white rounded-[2rem] font-black uppercase tracking-[0.3em] shadow-2xl hover:bg-sky-600 transition-all border-b-8 border-slate-900">
